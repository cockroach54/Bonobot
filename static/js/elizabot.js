...
ElizaBot.prototype.transform = function(text) {
	...
	// userLogMem에서 pastSize 이전 키워드까지 돌아봄
	for(var pp=0; pp<pastSize+1; pp++){
		if(!text) break;	// 페이즈2 초반에서 앞의 것 참조해야하면 xnone으로 보내기
		if(pp>0 && text.indexOf('?')>-1) continue; // 과거의 \\?, really\\?는 참조 안함

		var parts=text.split('.');		
		console.log('pointed parts\n', parts);
		...
		text = userLogMem[userLogMem.length-1-pp];
		console.log('****find at past memory:', pp+1,"****", '\n', text);
	}
	// nothing matched try mem
	// rpl=this._memGet();
	// if nothing in mem, so try xnone
	if (rpl=='') {
		this.sentence = parts_original[parts_original.length-1]; // xnone에서는 원본 마지막 문장 사용
...
ElizaBot.prototype._execRule = function(k) {
	...
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			if(reasmbs[ri][1]!=-1){	// (1)등 괄호 반영도 다 지우기
